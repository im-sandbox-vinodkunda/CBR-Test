...
{
    public class StrategicForecastExportToExcel
    {
        ...
        private static void WriteExcelFile(DataSet ds, SpreadsheetDocument spreadsheet, BankProfileIntroductionData bankdata, string modelName)
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                ...
                //{
                MergeCell mergeCell1 = new MergeCell() { Reference = new StringValue("A1" + ":" + "I1") };
                MergeCell mergeCell2 = new MergeCell() { Reference = new StringValue("A2" + ":" + "I2") };
                MergeCell mergeCell3 = new MergeCell() { Reference = new StringValue("A3" + ":" + "I3") };
                MergeCell mergeCell4 = new MergeCell() { Reference = new StringValue("A4" + ":" + "I4") };
                MergeCell mergeCell5 = new MergeCell() { Reference = new StringValue("A8" + ":" + "I8") };
                MergeCell mergeCell6 = new MergeCell() { Reference = new StringValue("A31" + ":" + "I31") };
                ...
                //    {
                //        maxLength = d.ToString().Length;
                //        if (maxLength > width1)
                //        {
                //            width1 = GetWidth(Convert.ToInt32(maxLength));
                //        }
                //    }
                //    if (width1 > 0)
                //    {
                //        width1 = GetWidth(Convert.ToInt32(width1));
                //        newWorksheetPart.Worksheet.InsertAfter(CreateColumnData((uint)l, (uint)l, width1 + 0.5), newWorksheetPart.Worksheet.SheetFormatProperties);
                //        l++;
                //    }
                //    else
                //    {
                //        newWorksheetPart.Worksheet.InsertAfter(CreateColumnData((uint)l, (uint)l, 13 + 0.5), newWorksheetPart.Worksheet.SheetFormatProperties);
                //        l++;
                //    }
                //}
                //}
                ...
                for (i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    {
                        if (width <= 13)
                            newWorksheetPart.Worksheet.InsertAfter(CreateColumnData((uint)i + 1, (uint)i + 1, 13), newWorksheetPart.Worksheet.SheetFormatProperties);
                        else
        ...
        private static void WriteDataTableToExcelWorksheet(DataTable dt, WorksheetPart worksheetPart, BankProfileIntroductionData bankdata, string modelName)
        {
            ...
            if (dt.TableName != "Summary")
            {
                ...
                AppendTextCell(excelColumnNames[0] + "3", "Saved Scenario:" + modelName, headerRow3, 8);
            }
            else
            {
                if (modelName != "na")
                {
                    var headerRow3 = new Row { RowIndex = 3 };  // add a row at the top of spreadsheet
                    sheetData.Append(headerRow3);
                    AppendTextCell(excelColumnNames[0] + "3", "Saved Comparison:" + modelName, headerRow3, 8);
                }
            }
            //var headerRow4 = new Row { RowIndex = 8 };  // add a row at the top of spreadsheet
            //sheetData.Append(headerRow4);
            //AppendTextCell(excelColumnNames[0] + "8", "" , headerRow4, 1);
            //}
            string colIndex = "1";
            if (dt.TableName == "Forecast")
            {
            ...
            for (int colInx = 0; colInx < numberOfColumns; colInx++)
            {
                ...
                DocumentFormat.OpenXml.UInt32Value index = 8;

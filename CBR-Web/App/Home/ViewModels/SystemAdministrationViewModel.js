...
cbrSystemAdministrationModule.controller("addAccountsController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$filter", "dialogs", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $filter, dialogs) {
    ...
    $scope.EnableDisableSearchBankButton = function ($event, accountType) {
        ...
        else if (accountType === 'Non-Bank' || accountType === 'F500Non-Bank' || accountType === 'CreditUnion') {
    ...
    $scope.IsUserDuplicate = function ($event) {
        if ($scope.Email !== null && $scope.Email.length > 0) {
            var inviteeDetails = {
                FirstName: btoa($scope.FirstName),
                LastName: btoa($scope.LastName),
                EmailAddress: btoa($scope.Email)
            };
    ...
    var IsInstitutionAvailable = function () {
        ...
        if (typeof $scope.SelectedInstitutions[0] !== 'undefined') {
            ...
            $http(req).then(function (result) {
                ...
                function () {
                    alert('An error occurred while trying to get active institutions. Please send an e-mail to castellonf@stifel.com.');
                });
        }
    };
    ...
    $scope.searchBanks = function (pageNumber) {
        if ($scope.BankFindSearchCriteria.AssetMinSize !== '' &&
            ...
            $http(req).then(function (result) {
                ...
                alert('An error occurred while searching for banks. Please send an e-mail to castellonf@stifel.com.');
                angular.element(document.querySelector('#searchLoaderEditPeerGroup')).html('');
            });
    ...
    $scope.AddAccount = function ($event, accountForm) {
        ...
        if ($scope.AccountType === 'Non-Bank' || $scope.AccountType === 'F500Non-Bank' || $scope.AccountType === 'CreditUnion') {
        ...
        $http(req).then(function (result) {
            if (result.data !== null && result.data === true) {
                ...
                angular.element(document.querySelector('#accountAddSuccess')).html('<span class="glyphicon glyphicon-info"></span> Failue! An error occurred while trying to create new account. Please send an e-mail to castellonf@stifel.com.');
            }
        },
            function () {
                alert('An error occurred while trying to get active institutions. Please send an e-mail to castellonf@stifel.com.');
        ...
        if ($scope.addNewsAndEventsForm.$valid) {
            ...
            $http(req).then(function (result) {
                if (result.data != null && result.data === true) {
                    ...
                    angular.element(document.querySelector('#accountAddSuccess')).removeClass('alert-success');
                    angular.element(document.querySelector('#accountAddSuccess')).addClass('alert-warning');
                    angular.element(document.querySelector('#accountAddSuccess')).html('<span class="glyphicon glyphicon-info"></span> Failue! An error occurred while trying to create news or event item. Please send an e-mail to castellonf@stifel.com.');
                }
            },
                function () {
                    alert('Failue! An error occurred while trying to create news or event item. Please send an e-mail to castellonf@stifel.com.');
                });
        }
    };
}]);
        ...
        if ($scope.addBlogForm.$valid) {
            ...
            $http(req).then(function (result) {
                if (result.data != null && result.data === true) {
                    ...
                    angular.element(document.querySelector('#accountAddSuccess')).removeClass('alert-success');
                    angular.element(document.querySelector('#accountAddSuccess')).addClass('alert-warning');
                    angular.element(document.querySelector('#accountAddSuccess')).html('<span class="glyphicon glyphicon-info"></span> Failue! An error occurred while trying to create blog item. Please send an e-mail to castellonf@stifel.com.');
                }
            },
                function () {
                    alert('Failue! An error occurred while trying to create blog item. Please send an e-mail to castellonf@stifel.com.');
                });
        }
    };
}]);
    ...
    $scope.UpdateImages = function () {
        ...
        $http(req).then(function (result) {
            if (result.data != null && result.data === true) {
                ...
                angular.element(document.querySelector('#updateImageMessageBox')).html('<span class="glyphicon glyphicon-info"></span> Failue! An error occurred while trying to update images. Please send an e-mail to castellonf@stifel.com.');
            }
        },
            function () {
                alert('Failue! An error occurred while trying to update images. Please send an e-mail to castellonf@stifel.com.');
            });
    };

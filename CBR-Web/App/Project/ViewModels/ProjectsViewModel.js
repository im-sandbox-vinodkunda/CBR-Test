cbrProjectsModule.controller("projectsNavigationController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$filter", "dialogs", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $filter, dialogs) {
        ...
        $http(req).then(function (result) {
            if (result.data !== "") {
               // setCookie('testcookie', result.data, 1);
                if (window.location.host.includes("cbr-qa") || window.location.host.includes("localhost") || window.location.host.includes("cb-resource-dev") || window.location.host.includes("cb-resource-auth")) {
                    window.open("https://ermtest.cb-resource.com/CBALanding/SSO");
                }
                else {
                    window.open("https://erm.cb-resource.com/CBALanding/SSO");
                }
            }
        });
    };
    ...
    var getTenantName = function () {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                var dlg = dialogs.error('An error occurred while trying to get accounts. Please send an e-mail to castellonf@stifel.com.');
            });
    };
        ...
        dlg.result.then(function (btn) {
            if (btn === 'yes') {
                ...
                $http(req).then(function (result) {
                    ...
                    function () {
                        var dlgerror = dialogs.error('An error occurred while trying to delete file. Please send an email to castellonf@stifel.com.');
                        document.getElementById('overlay').style.display = 'none';
                    });
            }
        }, function (btn) {

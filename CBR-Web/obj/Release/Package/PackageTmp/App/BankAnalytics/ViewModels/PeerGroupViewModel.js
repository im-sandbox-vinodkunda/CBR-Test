cbrBankAnalyticsModule.controller("peerGroupViewModel", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
    ...
    var getUsersCustomPeerGroups = function () {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                $scope.toggleErrorMessageBoxModal('An error occurred while trynig to get the custom peer groups. Please send an e-mail to admin@cb-resource.com.');
                angular.element(document.querySelector('#allPeerGroupsLoader')).attr('style', 'display:none');
            });
    }
...
cbrBankAnalyticsModule.controller("createPeerGroupViewModel", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
    ...
    var getLastFivePeerGroups = function () {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                $scope.toggleErrorMessageBoxModal('An error occurred while getting last five peer groups for user. Please send an e-mail to admin@cb-resource.com.');
            });
    }
        ...
        if ($scope.QuickStartForm.$valid) {           
                ...
                if (rssdNumbers.length > 150) {
                    ...
                    $http(req).then(
                        ...
                        function () {
                            $scope.toggleErrorMessageBoxModal('An error occurred while trying to create the peer group. Please send an e-mail to admin@cb-resource.com.');
                            document.getElementById('overlay').style.display = 'none';
                        });
                }
        }
...
cbrBankAnalyticsModule.controller("searchInstitutionViewModel", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$interval", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $interval) {
    ...
    $scope.createPeerGroup = function () {
        if ($scope.formpeergroup.$valid) {
            ...
            $http(req).then(
                ...
                function () {
                    document.getElementById('overlay').style.display = 'none';
                    $scope.toggleErrorMessageBoxModal('An error occurred while trying to create peer group. Please send an e-mail to admin@cb-resource.com.');
                });
        }      
        else {
            $scope.submitted = true;
    ...
    $scope.searchBanks = function (pageNumber) {
        ...
        $http(req).then(function (result) {
            ...
            $scope.toggleErrorMessageBoxModal('An error occurred while trying to search for banks. Please send an e-mail to admin@cb-resource.com.');
            angular.element(document.querySelector('#bankSearchLoader')).attr('style', 'display:none');
        });
    }
...
cbrBankAnalyticsModule.controller("editPeerGroupViewModel", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
    ...
    var getPeerGroupBasicDetails = function () {
        ...
        $http(req).then(function (result) {
            ...
            $scope.toggleErrorMessageBoxModal('An error occurred while trying to get peer group details. Please send an e-mail to admin@cb-resource.com.');
        });
    }
    ...
    var getPeerGroupMembers = function () {
        ...
        $http(req).then(function (result) {
            ...
            $scope.toggleErrorMessageBoxModal('An error occurred while trying to get peer group members. Please send an e-mail to admin@cb-resource.com.');
        });
    }
    ...
    $scope.createPeerGroup = function () {
        ...
        $http(req).then(
            ...
            function () {
                document.getElementById('overlay').style.display = 'none';
                $scope.toggleErrorMessageBoxModal('An error occurred while trying to edit peer group. Please send an e-mail to admin@cb-resource.com.');
            });
    }
...
cbrBankAnalyticsModule.controller("favoriteBanksViewModel", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
    ...
    $scope.RemoveBankFromFavoriteList = function ($event, bankObj) {
        ...
        $http(req).then(function (result) {
            ...
            $scope.toggleErrorMessageBoxModal('An error occurred while trying to remove the bank from favorite list. Please send an e-mail to admin@cb-resource.com.');
            document.getElementById('overlay').style.display = 'none';
        });
    }
    ...
    var getFavoriteBanks = function () {
        ...
        $http(req).then(function (result) {
            ...
            angular.element(document.querySelector('#favoriteBankLoader')).html('An error occurred while trying to get favorite banks. Please send an e-mail to admin@cb-resource.com.');
        });
    }

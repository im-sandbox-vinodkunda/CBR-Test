cbrAdministrationModule.directive('modal', function () {
    return {
...
cbrAdministrationModule.controller("adminNavigationController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
    $scope.adminTemplate = 'accountDetails';

    $scope.ToggleAdminScreen = function ($event, screenName) {
        $scope.adminTemplate = screenName;
    };
...
cbrAdministrationModule.controller("accountProfileController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$filter", "dialogs", "$uibModal", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $filter, dialogs, $uibModal) {
    $scope.Tenant = {};
    ...
    var getTenantProfile = function () {
        var req = {
            method: 'GET',
            url: '/api/AdminApi/GetTenantProfile',
            headers: {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                dlg = dialogs.error('An error occurred while trying to get account profile. Please send an e-mail to admin@cb-resource.com.');
...
cbrAdministrationModule.controller("manageUsersController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "dialogs", "$uibModal", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, dialogs, $uibModal) {
    $scope.TenantUsers = [];
    $scope.PageNumbers = [];
    $scope.PageSize = '10';
    $scope.CurrentPage = 1;
    ...
    var GetUserDetailsToEdit = function (userObj) {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                $scope.toggleErrorMessageBoxModal('An error occurred while trying to get users. Please send an e-mail to admin@cb-resource.com.');
            });
    };
    ...
    $scope.getTenantUsers = function (pageNumber) {
        ...
        var req = {
            method: 'POST',
            url: '/api/AdminApi/GetUsersForTenant',
            headers: {
                'Content-Type': 'application/json'
            },
            data: params
        ...
        $http(req).then(function (result) {
            ...
            function () {
                alert('An error occurred while trying to get users. Please send an e-mail to admin@cb-resource.com.');
            });
    };
...
cbrAdministrationModule.controller("editUserProfileController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$filter", "dialogs", "$uibModalInstance", "data", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $filter, dialogs, $uibModalInstance, data) {
    ...
    $scope.IsUserDuplicate = function ($event) {
        if ($scope.UserToEdit.email !== null && $scope.UserToEdit.email.length > 0) {
            var inviteeDetails = {
                ...
                EmailAddress: $scope.UserToEdit.email
            };
    ...
    $scope.UpdateUserProfile = function () {
        ...
        $http(req).then(function (result) {
            ...
            function () {
                dlg = dialogs.error('An error occurred while trying to update account profile. Please send an e-mail to admin@cb-resource.com.');
                dlg.result.then(function (btn) {
    ...
    var HideUnhideRoles = function () {
        ...
        if ($scope.Tenant.accountType === 'Bank' || $scope.Tenant.accountType === 'F500 Bank') {
...
cbrAdministrationModule.controller("addUsersController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", "$filter", "dialogs", "$uibModal", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope, $filter, dialogs, $uibModal) {
    $scope.NewUser = {
        firstName: '',
        lastName: '',
        title: '',
    ...
    $scope.AddUser = function (userForm) {
        var inviteeDetails = {
            ...
            Message: $scope.NewUser.message,
            Title: $scope.NewUser.title,
            TenantKey: $location.search().tenantkey
        };

        var req = {
            method: 'POST',
            url: '/api/AdminApi/InviteUser',
            headers: {
                'Content-Type': 'application/json'
            },
            data: inviteeDetails
        ...
        $http(req).then(function (result) {
            if (result.data !== null && result.data === true) {
                ...
                dlg = dialogs.error('An error occurred while inviting the user. Please send an e-mail to admin@cb-resource.com.');
                $scope.NewUser = {
                    firstName: '',
                    lastName: '',
                    title: '',
...
cbrAdministrationModule.controller("paymentMethodsController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
}]);

cbrAdministrationModule.controller("invoiceRecipientsController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
}]);

cbrAdministrationModule.controller("invoicesController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
}]);

cbrAdministrationModule.controller("closeAccountController", ["$scope", "$q", "$http", "$location", "$routeParams", "$window", "$rootScope", function ($scope, $q, $http, $location, $routeParams, $window, $rootScope) {
}]);

angular.bootstrap(document.getElementById("administration"), ['cbradministration']);
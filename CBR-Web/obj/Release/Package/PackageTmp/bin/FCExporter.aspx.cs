...
namespace CBR.Web
{
    public partial class FCExporter : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (isImgData)
                {
                    //Commented By Vinod on 16Nov2023
                    //convertRawImageDataToFile(exportData);
                    convertRawImageDataToFileAndUploadItToBlob(exportData);
                }
                else
                {
                    if (exportData["encodedImgData"] != null && exportData["encodedImgData"].ToString() != "" && ((Hashtable)exportData["parameters"])["exportformat"].ToString() == "svg")
                    ...
                    {
                        //Commented by Vinod for fixing the PDF issue
                        //exportObject = exportProcessor(((Hashtable)exportData["parameters"])["exportformat"].ToString(), "svg", (Hashtable)exportData["parameters"]);
                        exportObject = exportProcessor(((Hashtable)exportData["parameters"])["exportformat"].ToString(), "svg", (Hashtable)exportData["meta"]);
                    }
                }
            }
            ...
            //These changes included on 16Nov2023 by Vinod for the issue each chart export to jpg,png and some other exports are not working
            //To resolve this issue we have upgraded latest version of FustionCharts and Included licence key.
            //After upgradation FCExporter is not working, so we are modifying the code.
            //FusionChart latest version of zip flie donloaded from email link and copied only js files
            //Email subject "Your FusionCharts license key" on 15Nov2023
            //Pr to see these changes https://infomagnus.visualstudio.com/CB-Resource/_git/CB-Resource/pullrequest/2816
            object exportedStatus;
            if (exportData["parameters"] != null)
            {
                exportedStatus = outputExportObject(exportObject, (Hashtable)exportData["parameters"]);
            }
            else
            {
                exportedStatus = outputExportObject(exportObject, (Hashtable)exportData);
            }
        ...
        private Hashtable parseExportRequestStream()
        {
            ...
            {
                if (Request["stream_type"] != null && Request["stream_type"].ToString().ToLower() == "image-data")
                {
                    isImgData = true;
        ...
        //Added by Vinod on 16Nov2023
        private void convertRawImageDataToFileAndUploadItToBlob(Hashtable exportData)
        {
            Guid chartId = Guid.NewGuid();
            String fileName = "";
            CheckAndCreateDirectory("temp");
            fileName = Server.MapPath(".") + "\\temp\\" + chartId.ToString() + "." + exportData["exportformat"].ToString().ToLower();
            ...
            string chartFileName = exportData["exportfilename"].ToString();
            string chartFileFormat = exportData["exportformat"].ToString();
            
            UtilityFunctions.SaveOrUpdateChartMetadata(userKey, Convert.ToInt32(CommonFunctions.GetValueFromDescription<ChartTypeKeys>(chartFileName)), chartId);
            UtilityFunctions.UploadChartToAzureStorage(chartId.ToString() + "." + chartFileFormat, data);
        ...
        private object outputExportObject(MemoryStream exportObj, Hashtable exportParams)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    byte[] file = exportObj.ToArray();
                    long userKey = UtilityFunctions.GetUserKey(User.Identity.Name);
                    string chartFileName = ((Hashtable)(exportData["parameters"]))["exportfilename"].ToString();
                    string chartFileFormat = ((Hashtable)(exportData["parameters"]))["exportformat"].ToString();
                    Guid chartId = Guid.NewGuid();
                    UtilityFunctions.SaveOrUpdateChartMetadata(userKey, Convert.ToInt32(CommonFunctions.GetValueFromDescription<ChartTypeKeys>(chartFileName)), chartId);
                    UtilityFunctions.UploadChartToAzureStorage(chartId.ToString() + "." + chartFileFormat, file);
        ...
        private Hashtable setupServer(Hashtable exportParams)
        {
            ...
            retServerStatus["outStream"] = null;
    ...
    public class FCIMGGenerator
    {
        ...
        private Bitmap getImageObject(int id)
        {
            ...
            if (!rawImageData["bgcolor"].ToString().Contains("#"))
            {
                rawImageData["bgcolor"] = "#" + rawImageData["bgcolor"];
            }
            ...
            for (int yPixel = 0; yPixel < rows.Length; yPixel++)
            {
                ...
                for (int col = 0; col < color_count.Length; col++)
                {
                    ...
                    {
                        fRepeat = int.Parse(split_data[1]);
                    }
